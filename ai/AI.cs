...
namespace ai
{
    public static class AI
    {
        ...
        public static int[] NextMove(GameMessage gameMessage)
        {
            ...
            int depth = 4;
            int buffer = 2500; // in ms

            int[] nextMove = null;
            float score = float.MinValue;
            
            // Our basic loop runs the minimax search iteratively with increased depth
            // until the time remaining is below the buffer
            do
            {
                ...
                if (result.eval > score && sw.ElapsedMilliseconds < (time - buffer))
                {
                    score = result.eval;
                    nextMove = result.move;
                }
                depth++;
            } while (sw.ElapsedMilliseconds < (time - buffer));
        ...
        private static MoveResult MiniMax( int[][] boardState, int depth, float alpha, float beta, int player, ref System.Diagnostics.Stopwatch sw, ref int time)
        {
            ...
            // If the simulation has reached the end of the game or a depth of 0, evaluate
            if (CheckForThreshold(boardState, 0))
            {
            ...
            // Initialize the coordinates of the move to make. 9, 9 is in invalid move.
      ...
      public static List<int[]> GetPossibleMoves( int[][] board, int player )
        ...
        // Our board evaluation is based on three metrics used by research students at Cornell
        // 1. The difference in peices between the players, with little importance given
        // 2. The total amount of moves we could make with a given board
        // 3. The difference of corners between the two players
        ...
        // When the end of the game is reached by a search, it purely uses the game score
        // and multiplies it so that it will override previous board evaluations
        ...
        // This function checks if there are fewer free spaces than thresh
        // thresh is usually 0
